...
class UltrasonicManager:
    ...
    def run(self):
        while True:
            ...
            self.left_distance = self.left_values[len(self.left_values)-1] if self.left_values else 0  #(sorted(self.left_values)[len(self.left_values)//2]) if self.left_values else 0
            time.sleep(0.06)
            self.HandleUltrasonicData( self.right.distance(),self.right_values)
            self.right_distance = self.right_values[len(self.right_values)-1] if self.right_values else 0#(sorted(self.right_values)[len(self.right_values)//2]) if self.right_values else 0
